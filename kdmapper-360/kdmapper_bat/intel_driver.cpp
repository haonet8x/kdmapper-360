...
HANDLE intel_driver::Load() {
...
#ifndef bypass360
	if (!service::RegisterAndStart(driver_path)) {//360 Block ×¢²á±íÀ¹½Ø
	...
	if (!service::installDvr(driver_path)) {
		Log(L"[-] Failed to register and start service for the vulnerable driver" << std::endl);
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}
...
#else
	if (!service::UninstallDvr(GetDriverNameW()))
	{
		Log(L"[-] UnloadFailed" << std::endl);
	}
#endif
	std::wstring driver_path = GetDriverPath();
